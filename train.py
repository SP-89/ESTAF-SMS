...
def load_and_preprocess_data(data_dir, excel_path):
    ...
    for col in target_columns:
        if col in df.columns and df[col].isnull().any():
            ...
            print(f"Filled {col}  {df[col].isnull().sum()} missing values with median: {median_val:.2f}")
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            if psd_features.shape != (100, 29, 90):
                print(f"Warning: {patient_id} Feature shape {psd_features.shape}，not expected (100, 29, 90)")
                continue

            # Preserve temporal window structure - (100, 29*90)
            ...
            print(f"ProcessingFile {h5_file} Error:: {e}")
            continue

        current_idx += 1

    # Convert to numpy arrays
    X = np.array(all_features)  # : (samples, 100, 2610)
    ...
    # Reshape to temporal format (samples, , )
...
class ESTAF_SMS(nn.Module):
    ...
    def _init_weights(self):
        """Initialize weightsfor stable training"""
    ...
    def forward(self, x):
        ...
        # 1. CNN: (batch, features, seq_len)
        x = x.transpose(1, 2)

        # 2. CNN
        x = self.conv1(x)  # (batch, 16, 50)

        # 3. LSTM: (batch, seq_len, features)
...
def main(data_dir, excel_path):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y_stratify)):
        ...
        if fold_score > best_overall_score:
            ...
            print(f"New best modelsaved fold {fold + 1}, Average score: {fold_score:.4f}")
    ...
    print(f"\n fold {best_fold}，Average score: {best_overall_score:.4f}")
    ...
    for i, col in enumerate(target_columns):
        if col not in result_df.columns:
            print(f"Warning: column {col} not foundDataFrame，skip")
            continue
...
if __name__ == "__main__":
    ...
    excel_file_path = "metadata.xlsx"  # Excel file path
